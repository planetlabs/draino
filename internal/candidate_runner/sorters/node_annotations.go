...
const (
	// NodeAnnotationDrainPriorityPKey The annotation can have a numeric value associated. The higher the value, the higher the priority
	...
	NodeAnnotationDrainPriorityPKey = "node-lifecycle.datadoghq.com/drain-priority"
)

func CompareNodeAnnotationDrainPriority(n1, n2 *v1.Node) bool {
	var err error
	a1, a2 := 0, 0
	if n1.Annotations != nil {
		if str, ok := n1.Annotations[NodeAnnotationDrainPriorityPKey]; ok {
			a1 = 1
			if str != "" {
	...
	if n2.Annotations != nil {
		if str, ok := n2.Annotations[NodeAnnotationDrainPriorityPKey]; ok {
			a2 = 1
			if str != "" {
